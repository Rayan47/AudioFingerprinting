...
pub fn save_spectrogram_peaks(
    ...
    for (x, column) in spectrogram.iter().enumerate() {
        for (y, &val) in column.iter().enumerate() {
            ...
            let color1 = colorous::INFERNO.eval_continuous(1f64);
            let color2 = colorous::INFERNO.eval_continuous(0f64);
            ...
            if pixel_y < height as u32 {
                if val <= threshold {
                    imgbuf.put_pixel(x as u32, pixel_y, Rgb([color2.r, color2.g, color2.b]));
                }else{
                    imgbuf.put_pixel(x as u32, pixel_y, Rgb([color1.r, color1.g, color1.b]));
                }
